...
| **Pagos** | ✅ LIVE | `BlufinPagosPage.tsx` · `PagoModal.tsx` · `ForwardModal.tsx` · `BlufinPagoMultiplePage.tsx` · `pagos-queries.ts` | Sub-tabs Pendientes / Realizados / Forwards. **Pendientes rediseñado por semana** (feedback 2026-06-18): la vista ya NO agrupa por contrato sino que separa con un **toggle Anticipos / Saldos** (con conteo) y, dentro del tipo elegido, **agrupa los pagos por semana de su fecha programada** (`anticipo_fecha` / `saldo_fecha`). Grupos en orden: **Atrasado** (rojo — lo no pagado de semanas previas, lunes de su semana < lunes de hoy; es la "lista de lo pendiente que rodó") → **Esta semana** (azul, resaltado) → semanas futuras ("Semana del DD-mmm al DD-mmm") → **Sin fecha programada** (gris). Semana = lunes-domingo en **hora local** (helpers `lunesDe`/`isoLocal`/`addDias` en la página). Cada grupo es un card con cabecera (punto de color + label + nº pagos + total USD) y filas (folio + status · monto + "Programado <fecha>" · vencimiento/badge forward · botón Pagar/Pagar spot). El **folio es azul y clicable** → abre `SkusContratoModal` con los productos del contenedor (mismo modal y patrón que Llegadas → Por producto), y junto a "Programado <fecha>" va **en letra chica de qué es el contenedor** (producto principal · talla · "+N más", recortado con ellipsis y `minWidth:0` para no crecer la fila ni mover las proporciones del grid) — feedback 2026-08-06. Arriba, un `StatStrip` resume **"$ esta semana"** (azul) y **"$ atrasado"** (rojo) para responder de un vistazo "¿qué tengo que pagar esta semana?". El botón Pagar conserva la lógica de forward (badge "FORWARD CERRADO PARA …" + "Pagar spot"). **3 modos de captura**: (1) Pago individual vía `PagoModal` con auto-fill monto + auto-update flag contrato. (2) Forward vía `ForwardModal` con TC pactado + fecha entrega futura. (3) Pago múltiple vía `BlufinPagoMultiplePage` — página dedicada con checkbox-multiselect, TC/banco/fecha compartidos, override de monto por fila, sticky footer con totales, mutación batch `createPagosMultiples`. **Ejecutar forward**: botón "Ejecutar" en sub-tab Forwards convierte forward Pendiente en pago real (`executeForward`) — inserta pago con TC pactado + referencia `FORWARD ejecutado <fecha>` + cambia status a Ejecutado + recalcula flag. **Bloqueos**: 1 solo forward Pendiente por (contrato, asociado_a) — opción "Cubre anticipo/saldo" se deshabilita en modal si ya existe. **Pendientes con forward** muestran badge "FORWARD CERRADO PARA <fecha entrega>" y CTA cambia a "Pagar spot" para distinguir pago al TC del día vs ejecución del forward. **Pago spot libera el forward** (feedback 2026-06-13): cuando un pago spot cubre el tipo, `liberarForwardsCubiertos` pasa los forwards Pendientes de ese tipo a status **`Liberado`** — quedan cerrados con el banco pero ya no asignados al contenedor (no generan doble pago si después se "ejecutan"). La pestaña Forwards muestra badge gris "Liberado" sin botón Ejecutar; `executeForward` rechaza forwards no-Pendientes y además bloquea si el tipo ya está cubierto. **Validación anti doble-pago** (`validarNuevoPago` en create/múltiple, mismo criterio de "cubierto" que los flags): no se puede pagar un anticipo/saldo ya cubierto ni un contrato ya saldado — lanza toast con mensaje claro. **Saldo = lo que falta** (feedback 2026-06-14): el monto sugerido para "Saldo" ya no es el 90% fijo sino `total − todo lo pagado` (cubre: todo, resto tras anticipo, o resto tras abono); el modal muestra "Falta por pagar". **Forwards: mover y consumir parcialmente** (rediseño 2026-08-11, feedback "cerramos un forward para un contenedor pero se usó en otro"). El forward dejó de ser propiedad de un contenedor: es dinero pactado con el banco que se aplica donde de hecho se usó. **Vocabulario de `status`** (columna `text` libre; la fuente de verdad es `FORWARDS_ASIGNABLES` en `pagos-queries.ts`): `Pendiente` (asignado, ejecutable) · `Ejecutado` · **`Remanente`** (sobró al ejecutarlo contra un saldo menor; sigue vivo con el banco y es asignable) · `Liberado` (el contenedor se pagó spot) · **`Movido a …`** (`moverForwardAModulo`, 2026-08-11): el forward se **mueve de verdad al otro módulo** — se crea allá una fila **por asignar** (sin contenedor/factura) con el mismo monto, TC, banco y fechas + `origen_modulo='blufin'`/`origen_ref=<folio>`, y el de Blufin queda como **historial** (ya no ejecutable ni asignable). Migración `20260811130000`: `cam_forwards_sa` ganó **`empresa_id`** propio (antes se derivaba del contenedor con `!inner`, así que un forward sin contenedor era INVISIBLE) + `origen_modulo`/`origen_ref`; y se creó **`crm.nep_forwards`** (Neptuno no tenía forwards) colgando de la **factura**, con su pestaña Forwards completa en `NeptunoPagosPage` (`forwards-queries.ts`: fetch/create/asignar/ejecutar/borrar, con el mismo `min(forward, faltante)` y `Remanente` que Blufin). En los dos destinos, `factura_id`/`contenedor_id` NULL se pinta como badge **"Por asignar"** con botón **Asignar** (`CamSAAsignarForwardModal`, `NepAsignarForwardModal`). El insert en el destino va ANTES de marcar el origen: si el 2º paso falla, el peor caso es un forward visible en los dos lados (evidente y corregible) en vez de dinero que desaparece de ambos. **Asignar** ya no exige `Liberado`: sirve para los 3 status asignables (antes el botón era código muerto — nunca hubo un forward `Liberado` en la base). `reassignForward` **rechaza mover un `Ejecutado`** (movería el `contrato_id` dejando el pago huérfano en el contenedor viejo = doble pago) y su UPDATE lleva `.in('status', …)` para cerrar la carrera entre dos pestañas. **Ejecutar paga `min(monto del forward, faltante REAL del destino)`** (`planForward` calcula el plan y `executeForward` lo aplica — misma función para la previsualización y para la escritura, así no pueden discrepar): si NO alcanza el pago va como **`abono`** (no marca el saldo como cubierto porque `acumSaldo` solo suma pagos tipo `saldo`, pero sí cuenta para el total); si **excede**, se paga el faltante y el forward **se encoge en su lugar** a `Remanente` con el sobrante (fila nueva duplicaría el instrumento frente al banco). El **MXN del remanente se saca por RESTA**, no multiplicando, para no perder centavos al partir. Como no hay transacción, la referencia del pago lleva `fwd:<id>` y sirve de **clave de idempotencia**: si el insert pasó y el UPDATE falló, reintentar retoma en vez de duplicar. `EjecutarForwardModal` confirma antes (antes se ejecutaba sin preguntar) y la tabla avisa `excede $X` / `abono · falta $X` desde la lista. `deleteForward` ahora bloquea los `Ejecutado`. Verificado: con los 12 forwards pendientes reales el cálculo nuevo da "exacto", o sea cero cambio de comportamiento. **Pago múltiple: el saldo también es el REAL** (fix 2026-09-02, feedback del usuario "me voy a la parte de saldos y no coinciden con el saldo que es del contenedor"). `BlufinPagoMultiplePage` seguía ofreciendo cobrar `contrato.saldo_usd` — el saldo **programado** al crear el contrato (= total − anticipo) — mientras que Pagos → Pendientes ya usaba el real desde 2026-07-02: las dos pantallas mostraban números distintos del mismo contenedor. Ahora usa `fetchSaldosPorContrato` con la **misma fórmula** `total − pagado − NCs` y **oculta** el renglón si no queda nada (≤ $0.01). Impacto medido al aplicarlo: **38 de 57 renglones traían el monto mal**, todos hacia abajo — el total pasó de $4,382,810.30 a **$4,711,179.04**. La causa dominante (37 casos) era el **anticipo sin pagar**: el saldo programado lo daba por cobrado, así que ofrecía cobrar el 90% de un contenedor donde se debía el 100%; el caso restante (`MCO-CV-003687`) tenía pago parcial + NC y no restaba ninguno. ⚠️ **El cambio obligó a hacer excluyentes el anticipo y el saldo del mismo contrato**: como el saldo ya incluye el anticipo pendiente, seleccionar los dos cobraría **110%** del contenedor — y `createPagosMultiples` NO lo detecta solo, porque `validarNuevoPago` evalúa cada renglón contra el estado **anterior** al batch y ninguno se entera del otro. Se ataja en tres capas: el checkbox apaga a su hermano, "seleccionar todo" se queda con el saldo, y la mutación lanza error si aun así llegan juntos. El renglón de saldo que trae un anticipo dentro lo dice en ámbar ("incluye el anticipo"). De paso, `onSuccess` ahora invalida `blufin_saldos` (faltaba: al registrar pagos aquí, Pendientes se quedaba con saldos viejos). **Orden por vencimiento** (2026-09-03): la tabla de Pago múltiple se ordena por fecha de vencimiento ascendente (anticipos y saldos por igual, lo más urgente arriba), sin fecha al fondo; empate por folio y anticipo antes que saldo.

**Realizados** tiene filtros: search · chips tipo · select banco · rango fechas con suma filtrada en vivo. **Liquidado** = `saldo_pagado` (no requiere anticipo_pagado). **Flags centralizados en `recalcFlagsContrato`** (única fuente de verdad → `leerEstadoPago` + `cubiertos`; la usan create/delete/forward/múltiple **y NC**) con reglas: (a) el **saldo cubierto POR PAGOS** implica anticipo saldado (feedback 2026-06-11); (b) las **NCs aplicadas reducen lo que se debe imputándose al saldo** pero NO implican anticipo pagado (2026-06-15); (c) "saldado" = pagos+NCs cubren el total. Selector de contrato = **`Combobox`** buscable (escribir el número). El "falta por pagar" resta pagos **y NCs** (`fetchSaldosPorContrato`). **Delete con PIN**: cada row tiene botón trash → `DeleteConfirmModal` con PIN 4 dígitos; `deletePago` recalcula flags a la baja. Bloqueo: `deleteContrato` rechaza si tiene pagos o forwards |
...
- **Camanchaca SA** (USD): contenedores (folio `CAM-###` por secuencia BD; ETA bodega = ETA Manzanillo +7d con `eta_bodega_confirmada`), pagos sin anticipos (completo/abono) + forwards + costo de importación MXN a agencias + **"Comparación internación"** (% del FOB con semáforo), recepción, NC simplificada, central de costos (FOB + importación), calendario, productos.
...
- **Alta de contenedor — 3 ajustes (2026-08-19, feedback del usuario):** (a) **el folio interno ahora SE CAPTURA** en el formulario. Antes lo asignaba solo la BD con `next_cam_folio()`, que arranca en CAM-001 y se habría encimado con la numeración real del usuario (va en CAM-3xx); `sugerirFolioSA` propone el siguiente tomando el máximo existente +1 (ignora sufijos tipo `CAM-280.2`) y el campo es editable — si se deja vacío, la BD sigue aplicando su default. `createContenedorSA` traduce el `23505` a "El folio X ya existe". (b) La **OC del proveedor** se muestra en la lista bajo el folio (`Factura 38906 · 03-jun · OC 719532`) y **entra al buscador**. (c) El **orden por defecto de la lista pasó a ETA Manzanillo DESCENDENTE** (lo más reciente arriba). Ojo con el centinela: los contenedores sin ETA se ordenaban con `'9999-99-99'`, que en descendente los mandaba al TOPE — se cambió a cadena vacía para que caigan al fondo en los dos sentidos.
- **6 ajustes al operarlo (2026-08-19, feedback del usuario):**
  1. **Sin presentación Paletizado/Granel** — no aplica en Camanchaca. Fuera del alta y de la recepción (`presentacion_recibida` se guarda `null`); las columnas de BD se conservan por el histórico.
  2. **Folio interno prellenado y editable** — `sugerirFolioSA` lo propone escrito en el campo, no como texto de ayuda.
  3. **Status 100% calculado** — `statusContenedorSA` ahora deriva "Entregado" de **`llegada_real`** (lo que escribe la recepción), NO del campo `status` guardado, que dependía de que alguien lo pusiera a mano y podía mentir en ambos sentidos. El selector de Status **se quitó del alta** y se muestra como texto automático. Reglas: `llegada_real` → Entregado · hoy ≥ ETA → En Manzanillo · hoy < ETA → En tránsito · sin factura ni ETA → Planeado.
  4. **Central de costos: dos columnas** — "Costo sin importación" (FOB en pesos) y "Costo con importación" (FOB + agencias). La de **sin importación solo sale si el TC ya es firme**: contenedor **liquidado** (TC real ponderado de los pagos) o **con forward** (TC pactado; si hay pago parcial + forward se mezclan ponderados). Si sigue expuesto al tipo de cambio dice **"Pendiente"** en vez de inventar un precio. ⚠️ Ojo: `tcEfectivoSA` ANTES tomaba el TC de cualquier pago parcial — mismo error que Blufin corrigió en 2026-07-07; ahora exige liquidación completa. Verificado con datos reales: 27 liquidados dan $102.6M sin importación / $106.4M con importación, y los 4 recién capturados (sin pago ni forward) salen "Pendiente".
  5. **Catálogos de importaciones** (`/app/importaciones/catalogos`, botón en el picker): alta/renombrado/activar-desactivar de **navieras y bodegas**, compartidas por los 3 proveedores. **Sin borrado** — los embarques históricos las referencian por FK, así que se desactivan (migración `20260819130000` le dio `activo` a `navieras`, como ya tenía `bodegas`). Muestra cuántos embarques usa cada naviera antes de retirarla. **🐛 Fix 2026-09-15 (revisión pedida por el usuario "¿funcionan para las 3 pestañas?"):** la página prometía que desactivar quita del capturar, pero **Camanchaca lo ignoraba** — `fetchCatalogosSA` traía navieras y bodegas SIN filtrar `activo`, y `fetchCatalogosMX` igual con bodegas (había 9 bodegas desactivadas ofreciéndose). Ahora los tres fetch de captura filtran `activo=true` (Blufin bodegas ya lo hacía; a su fetch de navieras también se le puso por higiene). **Mapa real de consumo**: navieras solo las ofrece el alta de contenedor de **Camanchaca SA** (Blufin las recibe del calendario de Menita como texto — ninguna pantalla suya las pinta; Neptuno no lleva naviera por diseño); bodegas las usan Blufin (recepción/programar llegada) y Camanchaca SA+MX (Neptuno no captura bodega). El subtítulo de la página ahora lo dice tal cual.
  6. **PDF de la factura en la ficha del contenedor** — subir/ver/quitar (migración `20260819120000`: `cam_contenedores_sa.factura_pdf_path`). Bucket privado `documentos-importacion` en `camanchaca-sa/<folio>-<ts>.pdf`, se abre con URL firmada. `quitarFacturaPdfSA` limpia la referencia ANTES de borrar el objeto: si el borrado falla queda un archivo huérfano (inocuo) en vez de una ficha apuntando a un PDF inexistente.
- **3 mejoras de operación (2026-09-04, feedback del usuario):** (a) **Pagos → pestaña "Pendientes"** (nueva, es la default): agrupa los contenedores con saldo por **semana de su `fecha_vencimiento`** — Atrasado (rojo) → Esta semana (azul) → semanas futuras → Sin fecha — mismo patrón visual que Blufin → Pagos → Pendientes; cada renglón trae folio, status, **restante REAL** (total − pagado − NCs, sin anticipos porque SA no maneja), "Vence <fecha>", factura y OC en letra chica, badge "FORWARD CERRADO PARA <fecha>" si hay forward Pendiente asignado (el CTA cambia a "Pagar spot") y botón Pagar que abre el `CamSAPagoModal` prellenado; StatStrip con "$ esta semana" y "$ atrasado". (b) **Recepción muestra factura y OC** junto al número de contenedor: en "Por recibir" (renglón: `Factura 39341 · OC 728691 · <contenedor> · <naviera>`), en el Historial (bajo el folio) y en la ficha de la recepción — el embed de `fetchRecepcionesSA` ganó `factura, oc_proveedor` (+ el tipo en `database.ts`). (c) **$/kg con y sin importación**: Central de costos → Por contenedor ganó la columna "MXN/kg sin imp." (misma regla que su columna de totales: **solo con TC firme**, si no "Pendiente"; la de con importación sí estima en ámbar) y la **ficha del contenedor** ahora desglosa: Costo sin importación (FOB) · Costo importación · Costo con importación · **MXN/kg sin importación** · MXN/kg con importación. Ojo: la ficha calcula su TC con los pagos que existan (sin exigir liquidación), a diferencia de Costos — comportamiento previo que no se tocó.
- **Pestaña "Planeación" ELIMINADA (2026-09-02, decisión del usuario).** Era el primer paso del flujo original (§7a): capturar el calendario que Felipe mandaba por WhatsApp como orden planeada y luego "confirmarla" con la factura. En la práctica **nunca se usó**: `crm.cam_ordenes_planeadas` estaba en **0 filas** y **ningún contenedor** tenía `orden_planeada_id`, así que quitarla no dejó nada huérfano. Se borró `CamSAPlaneacionPage.tsx`, su ruta y pestaña, las 4 funciones `*OrdenPlaneada` de `sa-queries.ts` y el flujo `?planeada=<id>` del alta de contenedor (prefill de la OC, banner "Confirmando la orden planeada", `orden_planeada_id` en el insert y el marcado a `confirmado`). **La TABLA y la columna `cam_contenedores_sa.orden_planeada_id` se CONSERVAN** — vacías e inertes; borrarlas sería DDL destructivo sobre una BD compartida y no gana nada. El alta de contenedor sigue siendo el punto de entrada único. ⚠️ El **status `'Planeado'` NO desapareció**: `statusContenedorSA` lo sigue devolviendo cuando un contenedor no tiene factura ni ETA — es solo el fallback, y como el formulario exige la factura hoy no lo tiene ninguno de los 32.
...
- Lista con filtros (razón social/RFC del emisor, rango de fechas, chips de tipo de comprobante I/E/T/P, chips PUE/PPD y **select de forma de pago** — c_FormaPago, 11 claves, agregado 2026-07-31), tabla densa, paginada de 50 en 50, timestamp de "última actualización" (lee `cont_solicitudes`). ⚠️ **Los filtros se aplican en DOS lugares**: `fetchFacturas` (lista) y `fetchTodasLasFacturas` (export a Excel) tienen la misma cadena de `.eq()` duplicada — al agregar un filtro hay que tocar los dos o el Excel exporta de más. **Forma de pago**: los comprobantes de **Pago (P)** y **Carta porte (T)** no la traen en el encabezado (en los P vive en el complemento, `cont_pagos.forma_pago`), así que filtrar por ella los deja fuera — son 4,074 de ~14,800 facturas.
...
**Verificación de cancelaciones ✅ (2026-08-13)** — antes `estatus_sat` era una **constante**: el sync solo le pide comprobantes VIGENTES al SAT (`descarga.py`) y `cfdi_extract.py` escribía `"estatus_sat": "vigente"` a mano, así que el semáforo Vigente/Cancelado **nunca podía pintarse rojo** y una factura cancelada por el proveedor DESPUÉS de descargada se quedaba vigente para siempre. Ahora se le pregunta al SAT de verdad:
- **`sat-sync/sat_connector/estatus.py`** consulta el **servicio público** `ConsultaCFDIService` (el del QR impreso en cualquier CFDI, `?re=&rr=&tt=&id=`). **NO usa e.firma** — por eso `cli.py` importa `sync` de forma **perezosa** (dentro de cada comando): `sync` arrastra `satcfdi` y eso dejaría el comando `estatus` inutilizable donde esa librería no esté. Corre como paso extra del workflow (`run_estatus.py --max 1500`, `if: always()`), 3x/día por empresa → el padrón entero (~15 mil) rota cada 2-3 días. Migración `20260813120000`: `cont_facturas` += `estatus_codigo`/`estatus_cancelacion`/`es_cancelable`/`validacion_efos`/`estatus_verificado_at` + 2 índices.
- **Regla defensiva**: `estatus_sat` solo se mueve cuando el SAT contesta `CodigoEstatus` que empieza con **`S`**. Un `N - 602` o una racha de respuestas malas guarda el código crudo y estampa la fecha, pero **no degrada** el estatus — si no, un mal día del SAT marca media contabilidad como sospechosa.
- **El frontend distingue TRES casos, no dos** (`estatusMeta`): cancelada (rojo) · vigente verificada (verde) · vigente **sin verificar** (gris). Todas nacen "vigente" por construcción, así que pintarlas verdes antes de preguntar sería afirmar algo que no se sabe. Aviso arriba de la lista (`AvisoEstatus`) con canceladas + **cuántas ya tienen pago registrado** (el cruce que de verdad duele) + chips de filtro Todas/Vigentes/Canceladas.
- ⚠️ **`ValidacionEFOS`: 200 Y 201 son LIMPIAS.** El SAT no publica la tabla de este campo. La primera versión asumió "≠200 = empresa fantasma" y **marcó 96 facturas como no deducibles, incluyendo a TELÉFONOS DE MÉXICO** y una concesionaria de casetas. Se corrigió con datos + `phpcfdi/sat-estado-cfdi`, que mapea **200 y 201** a "fuera del listado" (`cfdiutils` documenta `100` = sí está). Es **lista blanca** (`EFOS_LIMPIOS`) a propósito: se marca lo que no esté ahí, para que un código desconocido pero grave no pase. Con la regla correcta: **0 alertas** en las 15,045 facturas (solo existen 200 y 201). El texto de la UI dice "verificar el RFC en el listado 69-B", nunca afirma que no sea deducible.
- ⚠️ **PostgREST corta en 1000 filas y NO avisa** (`db-max-rows` de Supabase): pedir `limit=11000` devuelve 1000 sin error. El primer barrido se quedó corto en silencio por esto; `facturas_por_verificar` ahora pagina con `offset`. Vale para cualquier consulta masiva del proyecto.
- ✅ De paso se **eliminó el footgun de los filtros duplicados**: la cadena de `.eq()` vivía copiada en `fetchFacturas` y en `fetchTodasLasFacturas` (export), y agregar un filtro solo en una hacía que el Excel exportara de más. Ahora las dos usan `queryFacturasFiltradas`.
- **Primer barrido (2026-08-13)**: 15,045 facturas verificadas, 0 errores → **13 canceladas** que el CRM daba por vigentes (11 PML + 2 Marlin), ninguna con complemento de pago ligado. Las más gordas: 2 de JESÚS ALEJANDRO RODRÍGUEZ CRUZ del 11 y 12-ago por **$320,100 y $309,900**, y una de MERCANTIL DE ALIMENTOS DEL MAR del 30-jul por $115,520.
- 📌 **No se portó al repo hermano `Contabilidad PML`** (que tiene su propia copia de `sat_connector/`). No hacía falta —la verificación corre en GitHub Actions desde ESTE repo— y las dos copias ya venían divergidas en `config/cli/supabase_sink/sync/storage`, así que portar a ciegas era más riesgo que valor. Si algún día se corre el verificador desde allá, hay que llevarse `estatus.py` + los 2 métodos de `supabase_sink` + el flag `requiere_efirma` de `config`.
...
4. ~~**Facturas canceladas después de usadas**~~ ✅ **HECHO 2026-08-13** — ver "Verificación de cancelaciones" arriba. Lo que **queda pendiente** de este punto: la alerta hoy vive dentro de la pantalla de Contabilidad (hay que entrar a verla). Falta empujarla — badge en el sidebar, correo, o notificación — para que una cancelación se entere sola. Y el cruce "cancelada con pago" solo detecta pagos vía **complemento (PPD)**; una PUE pagada por transferencia no queda ligada a nada en el CRM.
...
- **2026-06-22: motor de sincronización del Calendario de llegadas** (`scripts/sync_calendario.py`). El vendedor **Alfonso Gutiérrez (`alfonso.gutierrez@menita.com.mx`)** manda por correo, **martes y viernes**, un PDF "Calendario de llegadas" (`Calendario_Lizárraga_<fecha>.pdf`) con una tabla: CONTRATO (folio) · LOTE · **FECHA APROX. DE LLEGADA** (= ETA puerto, se mueve mucho) · FACTURA · PRODUCTO · MARCA · importes · STATUS · **CONTENEDOR** · **NAVIERA** (la "empresa"). El motor (pypdf + PostgREST) parsea el PDF, lo cruza por folio con `blufin_contratos` y **actualiza contenedor, naviera y eta_puerto**; si la ETA puerto cambia, **recalcula la eta_bodega estimada (+7d) solo si seguía siendo la estimada** (no pisa una oficial); **omite los recibidos** (`llegada_real != null`); y **reporta los folios del calendario que NO están en la base = "contratos nuevos para pedir"** (no los crea). Flags `--dry`/`--json`. **Aplicado el calendario del 19-jun (2026-06-22):** 24 contratos actualizados (4 con contenedor+naviera nuevos: 003662/003633/003561/003624; 20 con cambio de ETA), 15 sin cambio, 1 recibido omitido (003606), **0 nuevos** (los 40 ya existían). **Hallazgo Gmail (2026-06-22):** el MCP de Gmail SÍ detecta el correo (remitente + asunto "Calendario de llegadas" + nombre del adjunto PDF) pero **NO tiene herramienta para bajar el binario del adjunto** → para la automatización total el PDF debe aterrizar en **Google Drive** (vía Apps Script Gmail→Drive) y la rutina lo baja con el MCP de Drive. **Automatización ✅ LIVE (2026-06-22, "Automático total vía Drive"):** (1) Apps Script `guardarCalendarioMenita` (`scripts/apps_script_calendario.gs`, vive en la cuenta Google del usuario) busca `from:alfonso.gutierrez@menita.com.mx subject:"Calendario de llegadas" has:attachment` y guarda el PDF en la carpeta de Drive **"Calendario Menita CRM"** (id `19WhqL_qQm1Z6CAru9IdjPYzEh1PuIRgN`), idempotente por nombre, activador horario. (2) Tarea programada **`calendario-menita`** (LOCAL, `0 9 * * *`, mismo patrón que `facturas-menita-8am` — corre con los conectores Drive+Supabase de la sesión, sin riesgo de headless; SKILL.md en `~/.claude/scheduled-tasks/calendario-menita/`) lee el calendario más reciente de Drive, replica la lógica del motor (parse + diff + UPDATE vía Supabase MCP) y reporta cambios + el aviso de contratos nuevos. **Verificado end-to-end:** el Apps Script bajó 8 calendarios (26-may a 19-jun); idempotencia confirmada (reprocesar el 19-jun ya aplicado = 0 cambios). Ver [[project-facturacion-correo]] (mismo patrón Gmail→Drive→tarea local). **Ampliado 2026-08-06 — la FECHA PARA PAGO DEL SALDO también se sincroniza** (`saldo_fecha`): el usuario detectó que la rutina solo movía datos de llegada, y al comparar el calendario del 04-ago contra la base salió que **24 de 25 contenedores tenían la fecha de pago desalineada** (hasta 32 días: `003727` decía 25/08 y Menita 26/09). Importa porque **Pagos → Pendientes agrupa los saldos por semana usando `saldo_fecha`** — desalineada, el saldo cae en la semana equivocada. Reglas: se toma la **3ª fecha DD/MM/AAAA del renglón** (1ª = ETA puerto, 2ª = fecha de anticipo que **NO** se sincroniza, 3ª = "Fecha para pago"; los renglones "Contratada" no la traen); **NUNCA se toca si `saldo_pagado`** (es el registro histórico del pago); y **sí se evalúa en los recibidos** — un contenedor puede haber llegado y seguir sin liquidar ("Entregada por liquidar"), así que el candado de "recibido" ahora aplica solo a los datos de llegada, no a la fecha de pago. Aplicado a 21 contratos ese día. Actualizados a la par el `SKILL.md` de la tarea y `sync_calendario.py`.
- **2026-08-24: carga masiva real de 30 OC de Menita a staging** (lote `blufin_import_lotes` "Contratos Menita ago-2026 (30 OC)"). Primer uso productivo del flujo de carga masiva con contratos nuevos (el lote de jun-2026 era 100% duplicado). El usuario pasó 2 PDFs (`MCO-CV-003719-3727-3732 rev 1.pdf` y `MCO-CV-003734-3783.pdf`); se separaron en 30 PDFs (1 por folio, 2 páginas c/u) subidos a `documentos-importacion/menita-2026-08-24/` y se insertaron **30 contratos + 59 líneas** en staging. **28 listos para importar · 2 duplicados** (003727 y 003732 ya existían). Validado por SQL: los 30 cuadran en `Σ líneas == total_usd` y `anticipo + saldo == total`; 28 cuadran también en kg (003781/003782 traen 0.2 kg de redondeo en el SUBTOTAL del propio PDF de Menita). **Formato de estos PDFs** (distinto al de jun-2026, ojo con `scripts/split_contratos_pdf.py` que busca `NÚMERO:`): 3 páginas de contrato marco + cada OC en **2 páginas**, folio como `FOLIO: MCO-CV-######` **solo en la 1ª** → el split tiene que agrupar la página-con-folio + las siguientes hasta el próximo folio. **Mapeo de SKU**: se resolvió contra el histórico ya validado (`blufin_contrato_productos` agrupado por code) en vez de adivinar contra el catálogo — el catálogo **normaliza el `pct` a 100%** en varios SKUs mientras la OC trae el real (90%, 95%), así que comparar pct da falsos negativos. 55 líneas confianza alta, 5 media, 2 baja (duplicados), **1 sin SKU**: "Basa Rodaja 70% **100-150 Gr**" de 003753 — el catálogo solo tiene la talla 150/250 (`102005`), **falta ese SKU**. Marcas nuevas sin SKU propio de basa: **King Mark** y **Distribuidora del Pacífico** (003719/003760/003761 se mapearon al genérico `113004` con confianza media). **Errores de dedo en las OC de Menita** (anotados en `observaciones`, no corregidos — `importarLote` toma el kg/caja del catálogo): 003737/003738 traen `PESO/CAJA 1.00 kg` con 16,997 cajas (debe ser 4.54); 003756 lo mismo en aros de pota y surimi. **Las ETA de las OC son las originales** y difieren del calendario vigente (ej. 003719: OC 07/10 vs calendario 26/09) — al importar entran las de la OC y la siguiente corrida de la tarea `calendario-menita` las corrige. ✅ **Confirmado el mismo día**: tras importar los 28, el re-sync contra el calendario del 21-ago movió la `eta_puerto` de **los 28** (todas ~10 días antes de lo que decía la OC) y recalculó su `eta_bodega` estimada (+7d); segunda pasada = 0 cambios. **La `saldo_fecha` de esos 28 se queda con la FECHA LIQUIDACIÓN de la OC** hasta que Menita los publique con contenedor: mientras el renglón diga "Contratada" el calendario no trae fecha de pago, así que no hay de dónde sincronizarla — ojo en Pagos → Pendientes, algunas quedan antes de su propia ETA de puerto. **003727 llegó en "rev 1"** con un total distinto al que ya está en la base ($46,444.20 vs $48,192.10, precio $1.86 vs $1.93) — como es duplicado NO se importa; si el usuario quiere el precio nuevo hay que editarlo a mano. ~~**Pendiente**: del calendario del 21-ago siguen faltando los contratos 003788 y 003789~~ ✅ **resuelto 2026-08-27** — ver la nota de esa fecha abajo.
- **2026-08-27: los 2 contratos faltantes de Menita cargados a staging** (lote "Contratos Menita ago-2026 rev 2 (2 OC)"). El usuario pasó `MCO-CV-003788-3789 rev 2.pdf`; se separó en 2 PDFs de 2 páginas (`documentos-importacion/menita-2026-08-27/`) y se insertaron **2 contratos + 4 líneas**, 0 duplicados, las 4 líneas con SKU de confianza alta. Ambos son idénticos entre sí: $86,033.00 / 22,700 kg (18,001.10 kg de `102015` Tilapia Blufin 5/7 + 4,698.90 kg de `102014` 3/5, ambas a $3.79), anticipo $8,603.30 al 19-sep. **El mapeo de SKU se confirmó contra 003734/003735/003783**, que traen exactamente el mismo desglose — el catálogo normaliza el `pct` a 100% y la OC dice 95%, así que comparar pct habría dado falso negativo (mismo aprendizaje del 24-ago). **Las ETA de la rev 2 son MÁS NUEVAS que el calendario del 21-ago** (003788: OC 29/10 vs calendario 24/10; 003789: OC 10/11 vs 31/10) — el PDF se firmó el 26-ago. Al importar entran las de la OC y el siguiente calendario manda. `saldo_fecha` = FECHA LIQUIDACIÓN de la OC (13-nov y 25-nov) porque siguen "Contratada" y el calendario aún no trae fecha de pago. Entrega en **Durango** (`bodega_destino`, texto libre; no es de las 5 bodegas activas). **Falta que el usuario le dé Importar** en Contratos → Carga masiva.
