...
Con **Notas de crédito ✅ LIVE**, los tabs operativos de Blufin que faltan son:
- **Facturas** — subir PDF + comparador línea-por-línea vs contrato (necesita Storage bucket `facturas-pdf`, pendiente de infra #5).
...
- **Carga masiva de contratos PDF** — parser que mapea líneas del PDF contra el catálogo (pendiente de infra #2, Edge Function con LLM).
- Pendiente transversal: extender el borrador `useDraft` a `BlufinRecepcionRegistrarPage` y `BlufinPagoMultiplePage` (ver §17).
- Mejora NC: auto-disparar una NC "Sin monto" por presentación/faltante desde Recepción (hoy se captura manual).
