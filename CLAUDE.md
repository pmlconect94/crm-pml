...
- `crm.blufin_contratos`: 3 — `MCO-CV-100001` ($24,800), `MCO-CV-003876` ($74,456), `MCO-CV-003513` ($75,818). Los 3 con status **Entregado** y recepción registrada
- `crm.blufin_pagos`: 3 (~$99,392 USD). MCO-CV-003513 tiene anticipo pagado y **saldo pendiente** ($68,236.20) — único pendiente en Pagos
- `crm.blufin_recepciones`: 3 — destaca MCO-CV-003876 (FRIZAJAL, Intelisis 143580, lote 125654, CSAV) con diferencia de presentación **Paletizado → Granel** esperando NC (módulo de NCs aún no existe)
- Catálogo: 11 SKUs Blufin seedeados · `crm.bodegas` ampliada por el usuario (incluye FRIZAJAL)
