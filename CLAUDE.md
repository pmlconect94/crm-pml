...
| Auth | **Login propio correo+contraseña** (2026-06-23, `LoginPage.tsx` rediseñado tipo portada con logos Marlin+Lizárraga; **SSO Google/Microsoft y auto-registro ELIMINADOS** por decisión del usuario). 3 usuarios dados de alta a mano en `src/lib/auth.tsx` (`USUARIOS`, todos `admin_total`/ven todo): DIEGO DIAZ (ddl.pml2@gmail.com), ANA SILVIA (anasilvia_lizarraga@hotmail.com), JESUS (lizarragajesus@hotmail.com). **TRANSICIÓN**: `signIn(email,password)` hoy valida SOLO el correo contra esa lista (la contraseña aún NO) — falta migrar a **Supabase Auth real** (pendiente #3) para validar password encriptado + sesión; las contraseñas NO viven en el código. NO hay módulo de usuarios (el usuario los pide a mano, super personalizado). |
...
| 3 | **Supabase Auth real (correo+contraseña, SIN SSO)** — el login UI ya está LIVE (2026-06-23, sin Google/Microsoft); falta: (a) crear los 3 usuarios en Supabase Auth + filas en `crm.usuarios`, (b) que `signIn` use `supabase.auth.signInWithPassword` (validar password encriptado + sesión persistente, `persistSession:true`), (c) cargar rol/nombre desde `crm.usuarios`. Decisión 2026-06-23: nada de Google/Microsoft ni auto-registro. Requiere MCP de Supabase reconectado | Validación real de contraseña + RLS endurecida |
