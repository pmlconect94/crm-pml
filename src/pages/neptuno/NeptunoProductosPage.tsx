/**
 * Catálogo de productos Neptuno — CRUD del master de SKUs (proveedor='neptuno').
 * Se referencia en facturas y costos.
 */
...
import { fetchSkusNeptuno, toggleSkuActivo } from '@/features/neptuno/productos-queries';
import { exportProductos } from '@/features/neptuno/neptuno-export';
import { SkuModal } from '@/features/neptuno/SkuModal';
import type { CatalogoSku } from '@/types/database';

export function NeptunoProductosPage() {
  ...
  const { data: skus = [], isLoading } = useQuery({
    queryKey: ['neptuno_skus', empresaId],
    queryFn: () => fetchSkusNeptuno(empresaId),
  });
    ...
    <>
      <PageEnter className="hstack" style={{ justifyContent: 'space-between', marginBottom: 12 }}>
        <div>
          ...
          <p className="page-subtitle">Master de SKUs Neptuno — se referencia en facturas y costos</p>
        </div>
      ...
      <div className="card">
        {isLoading ? (
          ...
          <div className="empty">
            ...
            <div className="empty-title">{skus.length === 0 ? 'Catálogo vacío' : 'Sin SKUs que coincidan'}</div>
            <p className="muted">
              {skus.length === 0 ? 'Da de alta el primer producto del proveedor.' : 'Ajusta la búsqueda o el filtro de producto.'}
            </p>
            {skus.length === 0 && (
              ...
              {filtrados.map((s) => {
                ...
                return (
                  <tr key={s.id} style={inactivo ? { opacity: 0.55 } : undefined}>
                    <td className="mono fw-700" style={{ color: 'var(--blue-500)' }}>{s.code}</td>
                    ...
                    <td style={{ textAlign: 'right' }} className="mono fw-600">{fmtNum(s.kg_caja, 3)}</td>
                    <td>
                      {inactivo ? <span className="badge badge-red">Inactivo</span> : <span className="badge badge-green">Activo</span>}
                    </td>
                    <td>
                      <div className="hstack" style={{ gap: 4, justifyContent: 'flex-end' }}>
                        ...
                        <button
                          ...
                          title={inactivo ? 'Reactivar — vuelve a aparecer en capturas' : 'Desactivar — deja de aparecer en capturas (no se borra)'}
